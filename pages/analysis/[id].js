                ...
                {showScriptSection && (
                  <div className="space-y-8">
                    ...
                    {scriptContent && scriptContent.trim() && !scriptContent.trim().startsWith('{') ? (
                      <Card className={`overflow-hidden shadow-secondary border-white/5 bg-white/5 backdrop-blur-md transition-all duration-300 ${isScriptMaximized ? 'fixed inset-0 z-[100] rounded-none m-0 w-screen h-screen bg-[#0b0c15] flex flex-col' : ''}`}>
                        <div className="p-6 border-b border-white/5 bg-black/20 flex items-center justify-between flex-none gap-4">
                          <h2 className="text-lg font-bold text-white flex items-center gap-2 flex-1 min-w-0">
                            <div className="w-8 h-8 rounded-lg bg-green-500/20 text-green-400 flex flex-shrink-0 items-center justify-center border border-green-500/20">
                              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div className="flex items-center gap-2 md:gap-4 overflow-hidden">
                              ...
                              {analysis.metadata?.content_target && (() => {
                                const config = getPlatformConfig(analysis.metadata.content_target);
                                return (
                                  <Tooltip content={`Target Platform: ${analysis.metadata.content_target}`}>
                                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded border flex flex-shrink-0 items-center gap-1.5 h-6 ${config.color}`}>
                                      <span className="opacity-50 hidden md:inline">FOR</span>
                                      <span className="hidden md:inline">{analysis.metadata.content_target}</span>
                          ...
                          <div className="flex items-center gap-4">
                            {/* Character Count - Moved to right side */}

                            <Tooltip content={isScriptMaximized ? "Exit Fullscreen" : "Maximize Script"}>
                              <Button
                                size="sm"
                                variant="secondary"
                                onClick={() => setIsScriptMaximized(!isScriptMaximized)}
                                className={`h-8 md:min-w-[100px] border transition-all duration-200 font-semibold flex items-center justify-center gap-1.5 px-2 md:px-3 ${isScriptMaximized
                                  ? "!bg-indigo-600 !text-white !border-indigo-500 hover:!bg-indigo-500 hover:!border-indigo-400 shadow-lg shadow-indigo-500/20"
                            ...
                            {!isEditing && (
                              <Tooltip content="Edit Script">
                                <Button
                                  ...
                                  className="h-8 !bg-white/10 !text-white !border-white/20 hover:!bg-white/20 hover:!text-white hover:!border-white/30 md:min-w-[70px] font-semibold flex items-center justify-center px-2 md:px-3"
                                >
                                  <span className="hidden md:inline">Edit</span>
                                  <svg className="w-4 h-4 md:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </Button>
                              </Tooltip>
                            )}
                            ...
                            {isEditing && (
                              <div className="flex items-center gap-2">
                                <Tooltip content="Cancel Editing">
                                  <Button size="sm" variant="secondary" onClick={() => setIsEditing(false)} disabled={isSaving} className="h-8 !bg-white/10 !text-slate-200 hover:!text-white hover:!bg-white/20 !border-white/20 font-semibold flex items-center justify-center px-2 md:px-3">
                                ...
                                <Tooltip content="Save Changes">
                                  <Button size="sm" onClick={handleSave} disabled={isSaving} variant="gradient" className="h-8 font-semibold flex items-center justify-center px-2 md:px-3">
                                    <span className="hidden md:inline">Save</span>
                                    <svg className="w-4 h-4 md:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                  </Button>
                                </Tooltip>
                              </div>
                            )}
                        ...
                        <div className="bg-black/40 border-b border-white/5 px-6 py-3 flex items-center justify-between gap-4">
                          <p className="text-xs text-slate-400 font-medium">
                            This is a compressed, secondary version for clips, blogs, or summaries.
                          </p>
                          <span className="text-xs font-semibold text-slate-400 bg-white/5 px-3 rounded-lg border border-white/10 h-7 flex items-center justify-center whitespace-nowrap">
                            {(isEditing ? editableScript.length : (scriptContent || "").length).toLocaleString()} chars
                          </span>
                        </div>
                        <div className={`p-8 ${isScriptMaximized ? 'flex-1 overflow-y-auto' : ''}`}>
                          {isEditing ? (
                            <textarea
                              className={`w-full p-4 rounded-xl border border-white/10 bg-black/40 text-slate-200 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-mono text-sm leading-relaxed outline-none transition-all placeholder:text-slate-600 ${isScriptMaximized ? 'h-full resize-none' : 'h-[400px]'}`}
                              value={editableScript}
                              onChange={(e) => setEditableScript(e.target.value)}
                              placeholder="Enter script here..."
                            />
                          ) : (
                            <div className={`prose prose-invert max-w-none font-mono text-sm bg-[#0b0c15] text-slate-300 p-8 rounded-xl border border-white/10 whitespace-pre-wrap leading-relaxed shadow-inner ${isScriptMaximized ? 'min-h-full' : ''}`}>
                              {scriptContent}
                            </div>
                          )}
                        </div>
